...
correspond to components of the log-likelihood gradient.  Further automation
...
\item a set of contexts,\footnote{The set of contexts may be omitted
  ...
  and
\item for each context, a set of uniquely\NoteJE{what does this mean?} featurized events, including counts and visual positions.
\end{inparaenum}
...
but the tool works with recent versions of Firefox, Chrome and Safari,
...
Does their hypothesis space include the uniform distribution?  Does it
include every distribution?  How does the answer depend on the
selection of features?  When estimating the parameters, what can go
wrong?  \NoteJE{move the following to next section? }Can we design
regularized estimators that prevent overfitting?  What is the effect
of the regularization constant on large and small datasets?  What
...
The modeler has the onus of proper \textbf{feature design}.  Which
features usefully distinguish among the events?  When can a feature be
omitted because it provides no additional modeling power?  How does
the choice of features affect generalization, particularly if the
objective is regularized?
...
\ref{sec:history} or \newcite{smith-2011} for strategies on computing
the normalizer.
