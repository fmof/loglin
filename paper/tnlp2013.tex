...
Users with no statistical, computational or technical backgrounds found the tool helpful 
as well: they began to understand the models after working through the introductory 
lessons.
...
configuration files (see section~\ref{sec:tailoring}). This is useful for emphasizing specific concepts, 
considerations and applications; instructors may also 
use it to prepare students for any upcoming assignments. Section \ref{sec:history} provides  
some history and applications of log-linear modeling, as well as assignment ideas.
...
adding their own creative model \textit{features} that capture interesting 
\textit{attributes} or \textit{properties} of the data, 
...
The application is centered around lessons designed to isolate particular 
...
long as possible.\footnote{``Winning'' may not be immediately clear. As more advanced 
topics, such as the gradient, are introduced, knowing when you have won should become 
clear.}
...
These games are not disjoint, nor are they the only ones that can be played.
...
conjoined features are introduced. If a given set of features does not allow 
a particular dataset to be matched, 
...
\end{tabular}
\caption{Colors and area indicate differences betwen the model and empirical distributions (gray outlines, when applicable). For each 
observation, description or weight, red/small objects signify the proposed model under-predicts;  
...
the physical intuitions gained, and as a self-check on one's understanding. 
...
\NoteJE{Describe how tooltips encourage constructive play, perhaps
  ...
  solve).}
\NoteFF{tooltips help bridge intuition (reviewer's comment)}
...
We should note that we are \textit{not} concerned with computational issues here, i.e., that of 
...
manipulative that imparts a near-physical, intuitive understanding of log-linear models. See fn. \ref{fn:compute_partition_fn} 
or \newcite{smith-2011} for computing the partition function.
...
become progressively complex. Next we hide certain generating features from the students: they 
...
non-binary features and what it means for each game. 
...
%
%\Commented{
\section{History and Applications} \label{sec:history}
