...
taggers, and finite-state transducers.\NoteJE{Matt Post's stack decoder?}
...
Each shape $y$ is sized proportionately to its model
probability $p_{\vec{\theta}}(y \mid x)$ (defined as in
\eqref{eqn:loglin}), so it grows or shrinks as the user changes
$\vec{\theta}$.  
In contrast, the {\em empirical probability} or {\em count ratio}
\begin{equation}
\empirical\left(y\ \mid\ x\right) = \frac{N(x,y)}{N(x)},
...
Reinforcing this, the observed count $N(x,y)$ is shown at the upper
left of $y$, while the expected count $N(x)\cdot
...
maximize the (regularized) training log-likelihood. The log-likelihood
bar (Figure~\ref{fig:lesson1}) adapts to changes to $\vec{\theta}$,
...
\end{tabular}
\NoteJE{what is line 2 about?  I don't see the feature names changing
  color.}
