...
%Our aim is to provide an intuitive understanding of regularized conditional log-linear models. 
...
maximize the training log-likelihood. Because the log-likelihood is a distillation of how well 
the model predicts the training data. How long can you you make the bar? 

We represent the model's objective as a scalable bar graph. As shown in Figure \ref{fig:llbar}, we also visualize the regularization penalty, and the objective under the ``true'' (generating) model parameters, when known.
...
the shapes match, particularly when more advanced concepts such as regularization are 
...
\NoteJE{I might prefer a full screenshot
  rather than dividing the view up into pieces.} 
\NoteFF{The pagination is messing up. Figure 1 provides a screenshot,
	Figure 2 is meant to visually describe the size/color meanings.}
%\NoteJE{It's more organic.
%  The caption can then explain the color-coding in the example image,
%  just as our instruction to students do.  Remember that you are
%  talking to teachers here, not to HCI people---the goal is to
%  tell them what the student sees, rather than listing all of the nice
%  design elements, most of which can go unremarked (``it just
%  works'').}
