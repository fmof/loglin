...
object. 
A full survey of manipulatives' use in math education is beyond the scope of this paper; see the classic analysis from 
\newcite{sowell1989effects}, as well as the recent one by \newcite{carbonneau2013meta}, for more information 
on the effectiveness of manipulatives.
...
resulted in increased interest over the past two decades into \textit{virtual manipulatives}. 
Both \newcite{clements1996concrete} and \newcite{moyer2002virtual} provide accessible overviews of 
virtual manipulatives in early math education, but for our current purposes, the following highly simplified definition 
suffices: they are the virtual (e.g., web-based) analogue of concrete manipulatives. Virtual manipulatives give students the 
ability to affect some change upon a system and thus learn underlying properties \cite{moyer2002virtual}.

This latter point is most applicable here, 
since 
...
manipulatives in math: ``Learning and understanding mathematics, at every level, requires student engagement.'' 
National Library of Virtual Manipulatives\footnote{\url{nlvm.usu.edu/en/nav/vlibrary.html}} 

Members within the NLP community have previously explored this idea of ``teaching by doing.'' 
\newcite{eisner-2002-tnlp} presented a spreadsheet-based Hidden Markov Model.
\newcite{nouza1999teaching} explored a suite of interfaces and visualizations for various speech technologies 
\cite{light-tnlp-2005} developed web interfaces for a number of core NLP technologies and systems, such as parses, part-of-speech 
taggers, and finite-state transducers. They found this setup allowed students without the requisite background to experiment with and 
begin to learn about different NLP algorithms. 

\cite{halteren-tnlp-2002}  board game for teaching parsing
...
\NoteFF{FIX}
Contrasting conditional models and globally normalized ones
$p_{\vec{\theta}}\left(y\right)$. Although
these context-%nescient\NoteJE{???} 
unaware models allow students to focus on
...
\NoteFF{how does the penalty $C$ affect the overall solution?}

\NoteFF{what regularizers do we use?}
...
Underlying the above concepts is \textbf{general machine learning} and \textbf{statistical knowledge}. This includes the quality of estimators, 
can be affected by different data or model configurations, such as a small sample size leads to estimators with 
higher variance, but regularization can mitigate that (at the expense of the \textit{training} log-likelihood). 
Regularization can be considered a form of smoothing that fits the data imperfectly, but why? The model might 
has fewer parameters than outcomes, or it might forgo many parameters to avoid being penalized by 
the regularizer.  Further, to limit regularization penalty, the model can make use of backoff features and 
not just fine-grained features, thereby resulting in \textit{backoff smoothing}. 
These concepts are connected to the expressiveness considerations introduced in feature modeling.
