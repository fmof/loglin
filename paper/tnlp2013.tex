...
%
As discussed earlier, \eqref{eqn:reg_ll} is just the \textbf{difference between observed and
...
\label{eqn:obsexp} 
\end{equation}
minus a \textbf{weight decay} term for regularization.  Students must
internalize this concept and the computation of the two counts above.
...
Once we can compute the gradient, we can ``follow'' it along the
surface not only to yield a solution, but to yield a solution
guaranteed never to decrease our objective value.\footnote{We focus on
