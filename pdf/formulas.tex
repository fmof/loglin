...
average).  But unfortunately, it may have {\em high variance} (it is
sensitive to the particular dataset from which you obtain the counts).
It will be unreliable if either count is small:
\begin{itemize}[noitemsep]
\item If the denominator $\textrm{count}(x)=2$, then the estimate must be either 0/2, 1/2, or 2/2.  
\item It's even worse if the denominator is 1 or 0: then what happens?
\item If the numerator $\textrm{count}(x,y)=0$, then the estimate 
  be 0, but perhaps $y$ is just unlikely and not impossible.
\item If the numerator is 1, then the estimate might be too high,
  since perhaps there are many unlikely events and $y$ is just the one
  we happened to see.
\end{itemize}
...
independently of the others.\footnote{``Backoff smoothing'' does jointly
  estimate $p(y \mid x_1)$ and $p(y \mid x_2)$ if $x_1$ and $x_2$ are
...
Log-linear modeling is a very popular and flexible technique.  It has
the advantage that it works with {\em descriptions} of the events.
Contextualized events $(x,y)$ with {\em similar} descriptions tend to
...
Since you are the one who knows something about your problem, {\em
  you} get to define the function $\vec{f}$ that extracts those
...
Our model $\ph$ is now defined by\footnote{We are still using the
...
Equation~(\ref{eqn:loglin}) has to scale these unnormalized
probabilities, in order to get proper probabilities that sum to 1 (i.e., $\sum_y \ph(y\mid x) = 1$ for each $x$).  
This is a simple matter of dividing them by a 
{\bf normalizing constant}, namely
\begin{equation}
...
You can easily see that for any choice of $\vtheta$ and any $x$, equation~(\ref{eqn:loglin})
gives a genuine probability distribution over the events $y$ that
could occur in context $x$.  You can also see now why the model is
``log-linear''---for each context $x$, the log-probability is a linear function
...
$\vtheta$ specifies the linear coefficients and is incredibly
important: the way you pick it will determine all the probabilities in
the model, and even the normalizing constants!  Fortunately, you'll
have a training set of events in their contexts, $((x_1,y_1),
(x_2,y_2), \ldots (x_N,y_N))$.  Then the $\vtheta$ that maximizes
...
training data perfectly (a bad idea!).  This time we can only
manipulate the probabilities indirectly through the $\vtheta$
parameters of our log-linear model.
...
closely, which forces us to generalize.  You saw this in lesson 3.  On
the other hand, if the log-linear model has enough features, as in
lesson 4, then you can again fit the training data perfectly.
Maximizing (\ref{eqn:loglin-likelihood}) will then get you exactly the
same bad probabilities as in sction~\ref{sec:mle}.
