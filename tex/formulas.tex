...
\begin{align}
p(\text{solid circle}) & \defeq \frac{\exp\ (\theta_{\textrm{circle}}+\theta_{\textrm{solid}})\hspace{60mm}}{\exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{circle}}) + \exp\ (\theta_{\textrm{solid}}) + \exp\ 0} \\
\rule{0in}{1.5\baselineskip}
p(\text{striped circle}) & \defeq \frac{\exp\ (\theta_{\textrm{circle}})\hspace{1mm}}{\exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{circle}}) + \exp\ (\theta_{\textrm{solid}}) + \exp\ 0} \\
\rule{0in}{1.5\baselineskip}
p(\text{solid triangle}) & \defeq \frac{\hspace{46mm}\exp\ (\theta_{\textrm{solid}})}{\exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{circle}}) + \exp\ (\theta_{\textrm{solid}}) + \exp\ 0} \\
\rule{0in}{1.5\baselineskip}
p(\text{striped triangle}) & \defeq \frac{\hspace{82mm}\exp\ 0}{\exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{circle}}) + \exp\ (\theta_{\textrm{solid}}) + \exp\ 0}
\end{align}

Recall that $\exp\ x$ means $e^x$ where $e$ is a constant (namely
$e=2.71828\ldots$, the base of natural logarithms).\footnote{If
  $y=e^x$, then $x=\log y$.  That's the definition of $\log y$
  (sometimes written as $\log_e y$ or $\ln y$), 
...
\item $\exp x$ is always positive (except that $\exp(-\infty)=0$).  So
  all of the numerators and denominators above are $\geq 0$.  This
  makes all the probabilities $\geq 0$, as required.

\item As $x$ gets bigger, so does $\exp x$.  So raising
  $\theta_{\textrm{solid}}$ with your slider will raise the numerators
  of ``solid circle'' and ``solid triangle''.  To be precise, each
  time you increase $\theta_{\textrm{solid}}$ by 1, you multiply those
  numerators by $e$.  Subtracting 1 will divide them by $e$.

\item Finally, notice that the 4 probabilities sum up to 1, which is
  required because these are the only 4 possible outcomes:
\end{itemize}
\end{minipage}
\begin{equation}
  p(\text{solid circle}) + p(\text{striped circle}) + p(\text{solid circle}) + p(\text{striped triangle}) = 1
\end{equation}
(This was arranged by choosing a common denominator as shown---later
we'll call it ``$Z$.'')  So when the two solid probabilities
go up, the striped probabilities must go down to compensate.

\subsection{Formulas for Lesson 2}

Now every shape has exactly two features (whereas in lesson 1, some had fewer):

\begin{align*}
\hspace{-12mm}p(\text{solid circle}) & \defeq \frac{\exp\ (\theta_{\textrm{circle}}+\theta_{\textrm{solid}})\hspace{121mm}}{\exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{striped}}) + \exp\ (\theta_{\textrm{triangle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{triangle}} + \theta_{\textrm{striped}})} \\
\rule{0in}{1.5\baselineskip}
\hspace{-12mm}p(\text{striped circle}) & \defeq \frac{\exp\ (\theta_{\textrm{circle}}+\theta_{\textrm{striped}})\hspace{46mm}}{\exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{striped}}) + \exp\ (\theta_{\textrm{triangle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{triangle}} + \theta_{\textrm{striped}})} \\
\rule{0in}{1.5\baselineskip}
\hspace{-12mm}p(\text{solid triangle}) & \defeq \frac{\hspace{33mm}\exp\ (\theta_{\textrm{triangle}}+\theta_{\textrm{solid}})}{\exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{striped}}) + \exp\ (\theta_{\textrm{triangle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{triangle}} + \theta_{\textrm{striped}})} \\
\rule{0in}{1.5\baselineskip}
\hspace{-12mm}p(\text{striped triangle}) & \defeq \frac{\hspace{115mm}\exp\ (\theta_{\textrm{triangle}}+\theta_{\textrm{striped}})}{\exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{striped}}) + \exp\ (\theta_{\textrm{triangle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{triangle}} + \theta_{\textrm{striped}})} 
\end{align*}
