...
\subsection{Detailed derivation for the curious}
...
find that $\diffk F(\vtheta) = \sum_i f_k(x_i,y_i) - e_k(x_i)$.  We can regard this
as the total ``error'' in the observed features compared to what the
...
$y$ given $x$ is distributed according to the model.
...
\begin{align}
 ...
 &= - \diffj \underbrace{\sum_{y'} \left( p(y' \mid x) \cdot
   f_k(x,y'))}_{\textit{see \eqref{eqn:exp}}} \text{ i.e., the derivative of a linear combination} \\
 &= - \diffj e_k(x) 
