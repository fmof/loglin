...
in \cref{sec:loglin} that these formulas are examples of a more
...
probabilities \labelcref{eqn:solidcircle,eqn:solidtriangle} go
up, the striped probabilities \labelcref{eqn:stripedcircle,eqn:stripedtriangle} must go down to compensate.
...
of the $\exp$).  \Cref{eqn:loglin} has to scale them, in
order to get proper probabilities that sum to 1 (i.e., $\sum_y
p(y\mid x) = 1$ for each $x$).  As you can see in
\eqref{eqn:loglin}, this is a simple matter of dividing them by a {\bf
...
\cref{eqn:loglin} gives a genuine probability distribution
...
Look back at \cref{sec:concrete} of this
...
$f_4$ and $f_5$ from \cref{sec:featfuncs} would be defined more
formally as
\begin{align}
...
\cref{sec:featfuncs}, the first feature function might be
...
\medskip
In our example from \cref{sec:featfuncs}, $\vtheta \cdot \vec{f}(x,y)$ could be efficiently
computed as
\begin{algorithmic}
...
It turns out that maximizing \cref{eqn:loglin-likelihood} is
...
\eqref{eqn:loglin-likelihood}, or equivalently, the setting that
maximizes the logarithm of \eqref{eqn:loglin-likelihood}:\footnote{It
  just happens that the logarithm is easier to work with
  mathematically, because of the $\exp$ in \eqref{eqn:unnorm}.  But
...
\begin{align}
  ...
  &= \sum_{i=1}^N \diffk \log \ptilde(x_i, y_i) - \diffk \log Z(x_i) \\
  & \hspace{1in}\vdots \nonumber \\
...
defined in \cref{sec:formula}.  What does ``expected count'' mean in this
setting?  \eqref{eqn:obsexp} defined it as $\sum_{i=1}^N \sum_y
...
It can be shown that $F$ is a concave function (see \cref{app:hessian}), so it is like a simple hill with only one local maximum.  There are many reasonable ways to climb to the top of this hill.  One option is {\bf gradient ascent},\footnote{\label{fn:sgd}For large $N$, it is popular nowadays to use
...
(as in \cref{sec:notation}).  The true value would be denoted
by $\vtheta^*$.)

\subsection{Detailed derivation for the curious}\label{sec:bias-claim}

We got to \eqref{eqn:obsexp} above by applying basic rules
of differentiation:
\begin{align}
...
\Cref{eqn:obsexp} simply sums this over all training pairs $(x_i,y_i)$ to 
find that $\diffk F(\vtheta) = \sum_i f_k(x_i,y_i) - e_k(x_i) = O_k - E_k$.

\emph{Cool remark:} The function $-\frac{1}{N} F(\vtheta)$ gives the average surprisal of a training example.  Differentiating with respect to $\theta_k$, we get $\mean_i e_k(x_i) - f_k(x_i,y_i)$ (easy to see from the above).  This can be regarded as the model's \emph{bias}---its average error in predicting feature $k$ when using the current parameters.  And \cref{app:hessian} shows that differentiating twice with respect to $\theta_j$ and $\theta_k$, we get the \emph{covariance} of the model's predictions of features $j$ and $k$!
...
of \cref{sec:mle}, which also overfitted.  Both cases use {\bf
...
parameters.\footnote{Technically, the {\bf likelihood} of the
  ...
  \eqref{eqn:loglin-likelihood}, which says how probable the training
  ...
  likelihood \eqref{eqn:loglin-likelihood} or equivalently the
  log-likelihood \eqref{eqn:loglin-log-likelihood}.}

\subsection{How many parameters?}

What are the available parameters?  In \cref{sec:mle} we were
...
training data perfectly.  Maximizing \eqref{eqn:loglin-likelihood}
...
\eqref{eqn:loglin-log-likelihood}, let's maximize the
...
\end{equation}
This has a similar effect.  See \cref{app:more-regularization} for the differences.
...
\eqref{eqn:loglin-regularized}, provided that we define $p(\vtheta)$
...
\Cref{eqn:covar} is the negated covariance of $f_j(x,y')$
...
claim in \cref{sec:concave-claim} that $F(\vec{\theta})$ is
concave.

Alternatively, the Hessian of $-\frac{1}{N}F(\vtheta)$ can be 
regarded directly as a covariance matrix. To be precise, it gives the covariances
of the features $\vec{f}(x,y)$ when $x$ is distributed according to the
...
\cref{sec:bias-claim}, the function $-\frac{1}{N}F(\vtheta)$
...
We can check that \cref{eqn:covar} is correct:
\begin{align}
