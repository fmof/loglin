...
find that $\diffk F(\vtheta) = \sum_i f_k(x_i,y_i) - e_k(x_i)$.  Thus, 
$-\frac{1}{N} \diffk F(\vtheta)$ can be regarded as the model's \emph{bias}---its
average error in predicting feature $k$ with the current parameters.\footnote{And appendix~\ref{app:hessian} shows that $-\frac{1}{N} \diffj \diffk F(\vtheta)$ gives the \emph{covariance} of the model's predictions of features $j$ and $k$!}
...
% You can find more detailed presentations of this point in notes by
