...
\section{Starting out Concretely {\em [read with lessons 1 and 2]}}
...
p(\text{striped circle}) & \defeq \frac{\exp\ (\theta_{\textrm{circle}})\hspace{3mm}}{\exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{circle}}) + \exp\ (\theta_{\textrm{solid}}) + \exp\ 0} \\
\rule{0in}{1.5\baselineskip}
p(\text{solid triangle}) & \defeq \frac{\hspace{45mm}\exp\ (\theta_{\textrm{solid}})}{\exp\ (\theta_{\textrm{circle}} + \theta_{\textrm{solid}}) + \exp\ (\theta_{\textrm{circle}}) + \exp\ (\theta_{\textrm{solid}}) + \exp\ 0} \\
\rule{0in}{1.5\baselineskip}
...
\noindent All of these fractions have the same denominator, called $Z$.  It was chosen to make
\begin{equation}
p(\text{solid circle}) + p(\text{striped circle}) + p(\text{solid circle}) + p(\text{striped triangle}) = 1
\end{equation}

\noindent {\em Reminder:} $\exp v$ means $e^v$ where $e$ is a certain constant
(namely $e=2.71828\ldots$, the base of natural logarithms).  In
particular, $\exp 0=1$.

In lesson 1, some shapes have fewer than two features.  In lesson 2, every shape has two features:
